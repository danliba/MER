function [amp1,amp2]=fourier_amp(t1m,chlorf)
s1m=detrend(chlorf);
N1     = length(t1m);         % sample size
N21    = ceil(N1/2);          % half the number of observations (rounded up)
ts1    = t1m(1);              % first data point - Note: Phases will be                              % calculated relative to this point, not 0
...
fi1   = 1/(te1-ts1);          % the lowest non-zero frequency in the data                     % is also the frequency increment.
